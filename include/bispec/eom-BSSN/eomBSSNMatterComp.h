...
{
    return pfS(m,n) / (pfD(m,n) + pftau(m,n) + TINY_Real );
}
Real pfv_r( Int m, Int n )
{
    return ((pfS_r(m,n) * (pfD(m,n) + pftau(m,n)) - pfS(m,n) * (pfD_r(m,n)
    /*3*/  + pftau_r(m,n))) * pow2(r_minus(m,n))) / (r_plus(m,n) * pow2(pfD(m,n) + pftau(m,n)) + TINY_Real );
}
...
Real gtau_t( Int m, Int n )
{
    ...
    /*1*/  * gAlp_r(m,n) * pfS(m,n) * pow2(r_minus(m,n))) / (r_plus(m,n) * pow2(gA(m,n)) + TINY_Real )
    ...
    /*0*/  / (r_plus(m,n) * r(m,n) + TINY_Real );
}
Real gW( Int m, Int n )
{
    return (pfD(m,n) + pftau(m,n)) / ( sqrt(Power(pfD(m,n) + pftau(m,n),2) - (exp(-4
    /*3*/  * gconf(m,n)) * Power(pfS(m,n),2)) / ( Power(gA(m,n),2)  + TINY_Real )  ) + TINY_Real );
}
Real grhobar( Int m, Int n )
{
    return (exp(-6 * gconf(m,n)) * pfD(m,n) * sqrt(pow2(pfD(m,n) + pftau(m,n))
    /*2*/  - (exp(-4 * gconf(m,n)) * pow2(pfS(m,n))) / ( pow2(gA(m,n)) + TINY_Real )   )) / ((pfD(m,n)
    /*2*/  + pftau(m,n)) * gA(m,n) * pow2(gB(m,n)) + TINY_Real );
}
Real grho( Int m, Int n )
{
    return (exp(-6 * gconf(m,n)) * (pfD(m,n) + pftau(m,n))) / (gA(m,n)
    /*1*/  * pow2(gB(m,n)) + TINY_Real );
}
Real gj( Int m, Int n )
{
    return (exp(-6 * gconf(m,n)) * pfS(m,n)) / (gA(m,n) * pow2(gB(m,n))  + TINY_Real );
}
Real gJ11( Int m, Int n )
{
    return (exp(-2 * gconf(m,n)) * pfS(m,n) * pfv(m,n) * gA(m,n)) / ( TINY_Real + pow2(gB(m,n)) );
}
