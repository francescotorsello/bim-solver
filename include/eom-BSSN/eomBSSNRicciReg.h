...
{
    return (2 * gDA(m,n) * gDAlp(m,n)) / (3. * pow2(gA(m,n)) + TINY_Real) + (2 * gDB(m,n)
    /*1*/  * gDAlp(m,n)) / (3. * pow2(gA(m,n)) + TINY_Real) + (8 * gDconf(m,n) * gDAlp(m,n)) / (3.
    /*1*/  * pow2(gA(m,n)) + TINY_Real);
}
Real BimetricEvolve::eq_fDers( Int m, Int n )
{
    return (2 * fDA(m,n) * fDAlp(m,n)) / (3. * pow2(fA(m,n)) + TINY_Real) + (2 * fDB(m,n)
    /*1*/  * fDAlp(m,n)) / (3. * pow2(fA(m,n)) + TINY_Real) + (8 * fDconf(m,n) * fDAlp(m,n)) / (3.
    /*1*/  * pow2(fA(m,n)) + TINY_Real);
}
