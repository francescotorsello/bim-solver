...
{
    return gtrK_convr(m,n) + k_g * gJK(m,n) + exp(-4 * gconf(m,n)) * ((3 * gDers) 
    /*1*/  / 2. - (3 * (gDB(m,n) + 2 * gDconf(m,n)) * gDAlp(m,n)) / pow2(gA(m,n)) - (3 
    /*2*/  * gDAlp_r(m,n)) / pow2(gA(m,n))) + (2 * exp(-4 * gconf(m,n)) * gDAlpr_r(m,n)
    /*1*/  * r(m,n)) / pow2(gA(m,n)) - gtrA_pff(m,n) + (gAlp(m,n) * (3 
    /*2*/  * pow2(gA1(m,n)) + 6 * pow2(gA2(m,n)) + gtrK(m,n) * (2 * gtrA(m,n) 
    /*3*/  + gtrK(m,n)))) / 3.;
}
Real BimetricEvolve::eq_ftrK_t( Int m, Int n )
{
    return ftrK_convr(m,n) + k_f * fJK(m,n) + exp(-4 * fconf(m,n)) * ((3 * fDers) 
    /*1*/  / 2. - (3 * (fDB(m,n) + 2 * fDconf(m,n)) * fDAlp(m,n)) / pow2(fA(m,n)) - (3 
    /*2*/  * fDAlp_r(m,n)) / pow2(fA(m,n))) + (2 * exp(-4 * fconf(m,n)) * fDAlpr_r(m,n)
    /*1*/  * r(m,n)) / pow2(fA(m,n)) - ftrA_pff(m,n) + (fAlp(m,n) * (3 
...
Real BimetricEvolve::eq_gA1_t( Int m, Int n )
{
    ...
    /*0*/  * (gDers + gAlp(m,n) * ((2 * gDers * gDconf(m,n)) / gDAlp(m,n) - (8 
...
Real BimetricEvolve::eq_gA2_t( Int m, Int n )
{
    ...
    /*1*/  * gconf(m,n)) * (-gDers / 2. + gAlp(m,n) * (-((gDers * gDconf(m,n)) 
...
Real BimetricEvolve::eq_fA1_t( Int m, Int n )
{
    ...
    /*0*/  * (fDers + fAlp(m,n) * ((2 * fDers * fDconf(m,n)) / fDAlp(m,n) - (8 
...
Real BimetricEvolve::eq_fA2_t( Int m, Int n )
{
    ...
    /*1*/  * fconf(m,n)) * (-fDers / 2. + fAlp(m,n) * (-((fDers * fDconf(m,n)) 
...
Real BimetricEvolve::eq_gAsig_t( Int m, Int n )
{
    ...
    /*0*/  + (k_g * (gJA1(m,n) - gJA2(m,n)) + exp(-4 * gconf(m,n)) * ((3 * gDers) / 2.
    /*2*/  + gAlp(m,n) * ((-3 * gDers * (5 * gDB(m,n) + 11 * gDconf(m,n))) 
    /*3*/  / gDAlp(m,n) + (40 * gDB(m,n) * gDconf(m,n) + 7 * pow2(gDB(m,n)) + 44 
    /*4*/  * pow2(gDconf(m,n))) / pow2(gA(m,n)) + (27 * pow2(gDers) * pow2(gA(m,n))) 
    /*3*/  / (4. * pow2(gDAlp(m,n)))))) / pow2(r(m,n)) + (exp(-4 * gconf(m,n)) 
    /*1*/  * (-(gDAlpr_r(m,n) / pow2(gA(m,n))) + gAlp(m,n) * (gLr_r(m,n) + (2 
    /*4*/  * gsig_r(m,n) * pow2(gB(m,n))) / Power(gA(m,n),4) + (-2 * gDconfr_r(m,n) - 4
    /*4*/  * gDB(m,n) * gsig(m,n) + gsig_r(m,n) - gL(m,n) * gsig(m,n) * pow2(gB(m,n)))
    /*3*/  / pow2(gA(m,n))))) / r(m,n) + gAsig(m,n) * gAlp(m,n) * (gtrA(m,n) 
    /*1*/  + gtrK(m,n));
}
Real BimetricEvolve::eq_fAsig_t( Int m, Int n )
{
    ...
    /*0*/  + (k_f * (fJA1(m,n) - fJA2(m,n)) + exp(-4 * fconf(m,n)) * ((3 * fDers) / 2.
    /*2*/  + fAlp(m,n) * ((-3 * fDers * (5 * fDB(m,n) + 11 * fDconf(m,n))) 
    /*3*/  / fDAlp(m,n) + (40 * fDB(m,n) * fDconf(m,n) + 7 * pow2(fDB(m,n)) + 44 
    /*4*/  * pow2(fDconf(m,n))) / pow2(fA(m,n)) + (27 * pow2(fDers) * pow2(fA(m,n))) 
    /*3*/  / (4. * pow2(fDAlp(m,n)))))) / pow2(r(m,n)) + (exp(-4 * fconf(m,n)) 
    /*1*/  * (-(fDAlpr_r(m,n) / pow2(fA(m,n))) + fAlp(m,n) * (fLr_r(m,n) + (2 
    /*4*/  * fsig_r(m,n) * pow2(fB(m,n))) / Power(fA(m,n),4) + (-2 * fDconfr_r(m,n) - 4
    /*4*/  * fDB(m,n) * fsig(m,n) + fsig_r(m,n) - fL(m,n) * fsig(m,n) * pow2(fB(m,n)))
    /*3*/  / pow2(fA(m,n))))) / r(m,n) + fAsig(m,n) * fAlp(m,n) * (ftrA(m,n) 
    /*1*/  + ftrK(m,n));
}
