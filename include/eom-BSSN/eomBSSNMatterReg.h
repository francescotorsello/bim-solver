...
Real BimetricEvolve::eq_pf_gD_t( Int m, Int n )
{
    ...
    /*0*/  * (gBet_r(m,n) + 2 * gBetr(m,n) - gDAlp(m,n) * pfv(m,n) - gAlp(m,n)
    /*1*/  * pfv_r(m,n) - (2 * gAlp(m,n) * pfv(m,n)) / (TINY_Real + r(m,n)));
}
Real BimetricEvolve::eq_pf_gS_t( Int m, Int n )
{
    return pfS_convr(m,n) - gAlp(m,n) * pfS_r(m,n) * pfv(m,n) + pfS(m,n) * (2
    /*1*/  * gBet_r(m,n) + 2 * gBetr(m,n) - gDAlp(m,n) * pfv(m,n) + gAlp(m,n)
    /*1*/  * ((gDA(m,n) + 2 * gDconf(m,n)) * pfv(m,n) - pfv_r(m,n))) - gDAlp(m,n)
    /*0*/  * (pfD(m,n) + pftau(m,n)) - (2 * gAlp(m,n) * pfS(m,n) * pfv(m,n))
    /*0*/  / (TINY_Real + r(m,n));
}
Real BimetricEvolve::eq_pf_gtau_t( Int m, Int n )
{
    ...
    /*0*/  + (gBet_r(m,n) + 2 * gBetr(m,n) - gDAlp(m,n) * pfv(m,n) - gAlp(m,n)
    /*1*/  * pfv_r(m,n)) * pftau(m,n) - gAlp(m,n) * pfv(m,n) * pftau_r(m,n)
    /*0*/  - (gDAlp(m,n) * pfS(m,n)) / (TINY_Real + exp(4 * gconf(m,n))
...
Real BimetricEvolve::eq_pf_gW( Int m, Int n )
{
    ...
    ///*2*/  - (exp(-4 * gconf(m,n)) * Power(pfS(m,n),2)) / ( TINY_Real + Power(gA(m,n),2) )  ));
    return 1 / ( TINY_Real + Sqrt( 1 - exp(4*gconf(m,n)) * pow2( gA(m,n) ) * pow2(pfv(m,n)) ) );
}
Real BimetricEvolve::eq_pf_grhobar( Int m, Int n )
{
    ///return (pfD(m,n) * sqrt(pow2(pfD(m,n) + pftau(m,n)) - (exp(-4 * gconf(m,n))
    ///*3*/  * pow2(pfS(m,n))) / ( TINY_Real + pow2(gA(m,n)) )   )) / (TINY_Real + exp(6 * gconf(m,n))
    ///*1*/  * (pfD(m,n) + pftau(m,n)) * gA(m,n) * Power(gB(m,n),2));
    return pfD(m,n) / (TINY_Real + pfW(m,n) * exp(6 * gconf(m,n)) * gA(m,n)
    /*1*/  * Power(gB(m,n),2) );
}
