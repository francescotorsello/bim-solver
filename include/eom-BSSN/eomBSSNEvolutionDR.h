...
{
    return (gDsig(m,n) * gBet(m,n) + 2 * gBetr(m,n)) * gsig(m,n) + (pow2(gA(m,n)) * (2
    /*2*/  * gAsig(m,n) * gAlp(m,n) - (2 * gBetr_r(m,n)) / r(m,n))) / pow2(gB(m,n));
}
Real BimetricEvolve::eq_fsig_t( Int m, Int n )
{
    return (fDsig(m,n) * fBet(m,n) + 2 * fBetr(m,n)) * fsig(m,n) + (pow2(fA(m,n)) * (2
...
Real BimetricEvolve::eq_gAsig_t( Int m, Int n )
{
    ...
    /*1*/  * ((pow2(gsig(m,n)) * pow2(gB(m,n))) / Power(gA(m,n),4) + gDA(m,n) * (((2 
    ...
    /*3*/  * pow2(r(m,n))) + gsig(m,n) * (((gDsig_r(m,n) + pow2(gDsig(m,n)) - gDsig(m,n)
    /*5*/  * gL(m,n) * pow2(gA(m,n))) * pow2(gB(m,n))) / (2. * Power(gA(m,n),4)) - (4
    /*4*/  * gDB(m,n)) / (pow2(gA(m,n)) * r(m,n)) + (-(gL(m,n) * pow2(gA(m,n)) 
    /*5*/  * pow2(gB(m,n))) + gDsig(m,n) * (pow2(gA(m,n)) + 2 * pow2(gB(m,n)))) 
    /*3*/  / (Power(gA(m,n),4) * r(m,n))) + (gLr_r(m,n) - (2 * gDconfr_r(m,n)) 
    /*3*/  / pow2(gA(m,n))) / r(m,n)) + gAsig(m,n) * (gtrA(m,n) + gtrK(m,n)));
}
Real BimetricEvolve::eq_fAsig_t( Int m, Int n )
{
    ...
    /*3*/  * pow2(r(m,n))) + fsig(m,n) * (((fDsig_r(m,n) + pow2(fDsig(m,n)) 
    /*5*/  - fDsig(m,n) * fL(m,n) * pow2(fA(m,n))) * pow2(fB(m,n))) / (2. 
    /*4*/  * Power(fA(m,n),4)) - (4 * fDB(m,n)) / (pow2(fA(m,n)) * r(m,n)) + (-(fL(m,n)
    /*5*/  * pow2(fA(m,n)) * pow2(fB(m,n))) + fDsig(m,n) * (pow2(fA(m,n)) + 2 
    /*5*/  * pow2(fB(m,n)))) / (Power(fA(m,n),4) * r(m,n))) + (fLr_r(m,n) - (2 
    /*4*/  * fDconfr_r(m,n)) / pow2(fA(m,n))) / r(m,n)) + fAsig(m,n) * (ftrA(m,n) 
    /*2*/  + ftrK(m,n)));
}
Real BimetricEvolve::eq_gDsig_t( Int m, Int n )
{
    return gDsig_r(m,n) * gBet(m,n) * gsig(m,n) + gDsig(m,n) * (gBet_r(m,n) 
    /*1*/  * gsig(m,n) + 2 * gBetr(m,n) * gsig(m,n)) + gBet(m,n) * gsig(m,n) 
    /*0*/  * pow2(gDsig(m,n)) + (2 * gAsig(m,n) * gDAlp(m,n) * gAlp(m,n) 
    /*1*/  * pow2(gA(m,n))) / (TINY_Real + Power(gB(m,n),2)) + (2 * (gAsig_r(m,n) 
    /*2*/  * gAlp(m,n) * pow2(gA(m,n)) + 2 * gAsig(m,n) * (gDA(m,n) - gDB(m,n)) 
    /*2*/  * gAlp(m,n) * pow2(gA(m,n)) + gBetr_r(m,n) * gsig(m,n) * pow2(gB(m,n)))) 
    /*0*/  / (TINY_Real + Power(gB(m,n),2)) - (2 * (2 * gDA(m,n) * gBetr_r(m,n) - 2 
    /*2*/  * gDB(m,n) * gBetr_r(m,n) + gBetr_rr(m,n)) * pow2(gA(m,n))) / (TINY_Real 
    /*1*/  + Power(gB(m,n),2) * r(m,n)) + (2 * gBetr_r(m,n) * pow2(gA(m,n))) 
    /*0*/  / (TINY_Real + Power(gB(m,n),2) * Power(r(m,n),2));
}
Real BimetricEvolve::eq_fDsig_t( Int m, Int n )
{
    return fDsig_r(m,n) * fBet(m,n) * fsig(m,n) + fDsig(m,n) * (fBet_r(m,n) 
    /*1*/  * fsig(m,n) + 2 * fBetr(m,n) * fsig(m,n)) + fBet(m,n) * fsig(m,n) 
    /*0*/  * pow2(fDsig(m,n)) + (2 * fAsig(m,n) * fDAlp(m,n) * fAlp(m,n) 
    /*1*/  * pow2(fA(m,n))) / (TINY_Real + Power(fB(m,n),2)) + (2 * (fAsig_r(m,n) 
    /*2*/  * fAlp(m,n) * pow2(fA(m,n)) + 2 * fAsig(m,n) * (fDA(m,n) - fDB(m,n)) 
    /*2*/  * fAlp(m,n) * pow2(fA(m,n)) + fBetr_r(m,n) * fsig(m,n) * pow2(fB(m,n)))) 
    /*0*/  / (TINY_Real + Power(fB(m,n),2)) - (2 * (2 * fDA(m,n) * fBetr_r(m,n) - 2 
    /*2*/  * fDB(m,n) * fBetr_r(m,n) + fBetr_rr(m,n)) * pow2(fA(m,n))) / (TINY_Real 
    /*1*/  + Power(fB(m,n),2) * r(m,n)) + (2 * fBetr_r(m,n) * pow2(fA(m,n))) 
    /*0*/  / (TINY_Real + Power(fB(m,n),2) * Power(r(m,n),2));
}
