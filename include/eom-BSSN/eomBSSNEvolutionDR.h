/** @file  eomBSSNEvolutionDR.h
 *  @author Francesco Torsello
 *  @brief The double-regularized cBSSN evolution equations.
 *  @version 2018-11-08T09:18:33
 *  @image html BSSNevolutionDR.png
 */
...
{
    return gDsig(m,n) * (gBet_r(m,n) + 2 * gBetr(m,n)) * gsig(m,n) 
    /*0*/  + (gDsig_convr(m,n) + 2 * gBetr_r(m,n)) * gsig(m,n) + gBet(m,n) * gsig(m,n)
    /*0*/  * pow2(gDsig(m,n)) + (2 * (gAsig_r(m,n) + 2 * gAsig(m,n) * (gDA(m,n) 
    /*3*/  - gDB(m,n))) * gAlp(m,n) * pow2(gA(m,n))) / (TINY_Real + Power(gB(m,n),2)) 
    /*0*/  + (2 * gAsig(m,n) * gDAlp(m,n) * gAlp(m,n) * pow2(gA(m,n))) / (TINY_Real 
    /*1*/  + Power(gB(m,n),2)) - (2 * gBetr_rr(m,n) * pow2(gA(m,n))) / (TINY_Real 
    /*1*/  + Power(gB(m,n),2) * r(m,n)) + gBetr_r(m,n) * ((4 * (-gDA(m,n) + gDB(m,n)) 
    /*2*/  * pow2(gA(m,n))) / (TINY_Real + Power(gB(m,n),2) * r(m,n)) + (2 
    /*2*/  * pow2(gA(m,n))) / (TINY_Real + Power(gB(m,n),2) * Power(r(m,n),2)));
}
Real BimetricEvolve::eq_fDsig_t( Int m, Int n )
{
    return fDsig(m,n) * (fBet_r(m,n) + 2 * fBetr(m,n)) * fsig(m,n) 
    /*0*/  + (fDsig_convr(m,n) + 2 * fBetr_r(m,n)) * fsig(m,n) + fBet(m,n) * fsig(m,n)
    /*0*/  * pow2(fDsig(m,n)) + (2 * (fAsig_r(m,n) + 2 * fAsig(m,n) * (fDA(m,n) 
    /*3*/  - fDB(m,n))) * fAlp(m,n) * pow2(fA(m,n))) / (TINY_Real + Power(fB(m,n),2)) 
    /*0*/  + (2 * fAsig(m,n) * fDAlp(m,n) * fAlp(m,n) * pow2(fA(m,n))) / (TINY_Real 
    /*1*/  + Power(fB(m,n),2)) - (2 * fBetr_rr(m,n) * pow2(fA(m,n))) / (TINY_Real 
    /*1*/  + Power(fB(m,n),2) * r(m,n)) + fBetr_r(m,n) * ((4 * (-fDA(m,n) + fDB(m,n)) 
    /*2*/  * pow2(fA(m,n))) / (TINY_Real + Power(fB(m,n),2) * r(m,n)) + (2 
    /*2*/  * pow2(fA(m,n))) / (TINY_Real + Power(fB(m,n),2) * Power(r(m,n),2)));
}
