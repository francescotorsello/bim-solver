...
{
    return -gdet_pff_r(m,n) / (12. * gdet(m,n) * gconf(m,n)) + (gdet_pff(m,n)
    /*1*/  * gdet_r(m,n)) / (12. * gconf(m,n) * pow2(gdet(m,n))) - (gDAlp(m,n)
    /*1*/  * gAlp(m,n) * (gtrA(m,n) + gtrK(m,n))) / (6. * gconf(m,n)) + gDconf(m,n)
    /*0*/  * (gdet_pff(m,n) / (12. * gdet(m,n) * gconf(m,n)) + (6 * gconf(m,n)
    /*2*/  * gBet_r(m,n) + gAlp(m,n) * (gtrA(m,n) + gtrK(m,n))) / (6. * gconf(m,n)))
    /*0*/  + (6 * gconf(m,n) * gDconf_r(m,n) * gBet(m,n) - gAlp(m,n) * (gtrA_r(m,n)
    /*2*/  + gtrK_r(m,n))) / (6. * gconf(m,n));
}
Real BimetricEvolve::eq_fDconf_t( Int m, Int n )
{
    return -fdet_pff_r(m,n) / (TINY_Real + 12. * fdet(m,n) * fconf(m,n)) + (fdet_pff(m,n)
    /*1*/  * fdet_r(m,n)) / (TINY_Real + 12. * fconf(m,n) * pow2(fdet(m,n))) - (fDAlp(m,n)
    /*1*/  * fAlp(m,n) * (ftrA(m,n) + ftrK(m,n))) / (TINY_Real + 6. * fconf(m,n)) + fDconf(m,n)
    /*0*/  * (fdet_pff(m,n) / (TINY_Real + 12. * fdet(m,n) * fconf(m,n)) + (6 * fconf(m,n)
    /*2*/  * fBet_r(m,n) + fAlp(m,n) * (ftrA(m,n) + ftrK(m,n))) / (TINY_Real + 6. * fconf(m,n)))
    /*0*/  + (6 * fconf(m,n) * fDconf_r(m,n) * fBet(m,n) - fAlp(m,n) * (ftrA_r(m,n)
    /*2*/  + ftrK_r(m,n))) / (TINY_Real + 6. * fconf(m,n));
}
...
Real BimetricEvolve::eq_gL1_t( Int m, Int n )
{
    return k_g * gJL(m,n) + (-(gdet_pff(m,n) * gL1(m,n)) / 3. - gdet_pff_r(m,n) / (6.
    /*2*/  * pow2(gA(m,n)))) / gdet(m,n) + gBet_rr(m,n) / pow2(gA(m,n))
    /*0*/  + (gdet_pff(m,n) * gdet_r(m,n)) / (6. * pow2(gdet(m,n)) * pow2(gA(m,n)))
    /*0*/  + gBet(m,n) * (gL1_r(m,n) - 2 / (pow2(gB(m,n)) * pow2(r(m,n)))) + (4
    ...
    /*1*/  * pow3(r(m,n))) / (3. * pow2(gA(m,n))) + gBet_r(m,n) * (-gL1(m,n) + 2
    /*1*/  / (pow2(gB(m,n)) * r(m,n))) - (2 * gDAlp(m,n) * gAlp(m,n) * (3 * gA1(m,n)
    /*2*/  - gtrA(m,n))) / (3. * pow2(gA(m,n))) - (4 * gAlp(m,n) * (gtrA_r(m,n)
    /*2*/  + gtrK_r(m,n))) / (3. * pow2(gA(m,n)));
}
Real BimetricEvolve::eq_fL1_t( Int m, Int n )
{
    return k_f * fJL(m,n) + (-(fdet_pff(m,n) * fL1(m,n)) / 3. - fdet_pff_r(m,n) / (6.
    /*2*/  * pow2(fA(m,n)))) / fdet(m,n) + fBet_rr(m,n) / pow2(fA(m,n))
    /*0*/  + (fdet_pff(m,n) * fdet_r(m,n)) / (6. * pow2(fdet(m,n)) * pow2(fA(m,n)))
    /*0*/  + fBet(m,n) * (fL1_r(m,n) - 2 / (pow2(fB(m,n)) * pow2(r(m,n)))) + (4
    ...
    /*1*/  * pow3(r(m,n))) / (3. * pow2(fA(m,n))) + fBet_r(m,n) * (-fL1(m,n) + 2
    /*1*/  / (pow2(fB(m,n)) * r(m,n))) - (2 * fDAlp(m,n) * fAlp(m,n) * (3 * fA1(m,n)
    /*2*/  - ftrA(m,n))) / (3. * pow2(fA(m,n))) - (4 * fAlp(m,n) * (ftrA_r(m,n)
    /*2*/  + ftrK_r(m,n))) / (3. * pow2(fA(m,n)));
}
Real BimetricEvolve::eq_gsig_t( Int m, Int n )
{
    return gBet(m,n) * gsig_r(m,n) + (pow2(gA(m,n)) * (2 * gAsig(m,n) * gAlp(m,n) - (2
    /*3*/  * gBet_r(m,n)) / pow2(r(m,n)) + (2 * gBet(m,n)) / pow3(r(m,n))))
    /*0*/  / pow2(gB(m,n)) + (2 * gBet(m,n) * gsig(m,n)) / r(m,n);
}
Real BimetricEvolve::eq_fsig_t( Int m, Int n )
{
    return fBet(m,n) * fsig_r(m,n) + (pow2(fA(m,n)) * (2 * fAsig(m,n) * fAlp(m,n) - (2
    /*3*/  * fBet_r(m,n)) / pow2(r(m,n)) + (2 * fBet(m,n)) / pow3(r(m,n))))
    /*0*/  / pow2(fB(m,n)) + (2 * fBet(m,n) * fsig(m,n)) / r(m,n);
}
Real BimetricEvolve::eq_gAsig_t( Int m, Int n )
{
    ...
    /*4*/  * pow2(r(m,n)))) + (gL1_r(m,n) - (2 * gconf(m,n) * gDconf_r(m,n))
    ...
    /*3*/  / (pow2(gA(m,n)) * pow3(r(m,n)))) - gL1(m,n) / pow3(r(m,n)) + (2 * gconf(m,n)
    /*3*/  * gDconf(m,n)) / (pow2(gA(m,n)) * pow3(r(m,n))) + gsig(m,n) * ((-4
    /*4*/  * gDB(m,n)) / (pow2(gA(m,n)) * r(m,n)) - (gL1(m,n) * pow2(gB(m,n)))
    /*3*/  / (pow2(gA(m,n)) * r(m,n))) + gsig_r(m,n) * (-(gL1(m,n) * pow2(gB(m,n)))
    /*3*/  / (2. * pow2(gA(m,n))) + (pow2(gA(m,n)) + 2 * pow2(gB(m,n)))
    /*3*/  / (Power(gA(m,n),4) * r(m,n)))) + gAsig(m,n) * (gtrA(m,n) + gtrK(m,n)));
}
Real BimetricEvolve::eq_fAsig_t( Int m, Int n )
{
    ...
    /*4*/  * pow2(r(m,n)))) + (fL1_r(m,n) - (2 * fconf(m,n) * fDconf_r(m,n))
    ...
    /*3*/  / (pow2(fA(m,n)) * pow3(r(m,n)))) - fL1(m,n) / pow3(r(m,n)) + (2 * fconf(m,n)
    /*3*/  * fDconf(m,n)) / (pow2(fA(m,n)) * pow3(r(m,n))) + fsig(m,n) * ((-4
    /*4*/  * fDB(m,n)) / (pow2(fA(m,n)) * r(m,n)) - (fL1(m,n) * pow2(fB(m,n)))
    /*3*/  / (pow2(fA(m,n)) * r(m,n))) + fsig_r(m,n) * (-(fL1(m,n) * pow2(fB(m,n)))
    /*3*/  / (2. * pow2(fA(m,n))) + (pow2(fA(m,n)) + 2 * pow2(fB(m,n)))
    /*3*/  / (Power(fA(m,n),4) * r(m,n)))) + fAsig(m,n) * (ftrA(m,n) + ftrK(m,n)));
}
